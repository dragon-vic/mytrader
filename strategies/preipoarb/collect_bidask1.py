...
# 长期采集四个 preipo 标的的 websocket bid/ask1，并按小时合并 parquet。
async def main(duration_sec: int) -> None:
    ...
    stop = asyncio.Event()
    queue: asyncio.Queue[dict] = asyncio.Queue(maxsize=100_000)
    ...
    collectors = [
        asyncio.create_task(collect_binance(queue, stop, stop_at)),
        asyncio.create_task(collect_okx(queue, stop, stop_at)),
    ]
...
async def collect_binance(queue: asyncio.Queue[dict], stop: asyncio.Event, stop_at: float | None) -> None:
    streams = "/".join(f"{symbol.lower()}@bookTicker" for symbol in BINANCE_SYMBOLS)
    url = f"wss://fstream.binance.com/stream?streams={streams}"
    while not stop.is_set() and not expired(stop_at):
        try:
            write_log("binance connecting")
            async with websockets.connect(url, ping_interval=20, ping_timeout=20, close_timeout=5) as ws:
                write_log("binance connected")
                async for raw in ws:
            ...
            write_log(f"binance error {type(exc).__name__}: {exc}")
            await sleep_backoff(stop, stop_at)


# OKX bbo-tbt 推送最优一档，公共频道不需要鉴权。
async def collect_okx(queue: asyncio.Queue[dict], stop: asyncio.Event, stop_at: float | None) -> None:
    url = "wss://ws.okx.com:8443/ws/v5/public"
    args = [{"channel": "bbo-tbt", "instId": symbol} for symbol in OKX_SYMBOLS]
    while not stop.is_set() and not expired(stop_at):
        try:
            write_log("okx connecting")
            async with websockets.connect(url, ping_interval=20, ping_timeout=20, close_timeout=5) as ws:
                await ws.send(json.dumps({"op": "subscribe", "args": args}))
                write_log("okx connected")
                async for raw in ws:
                    if raw == "ping":
                        await ws.send("pong")
                        continue
            ...
            write_log(f"okx error {type(exc).__name__}: {exc}")
            await sleep_backoff(stop, stop_at)
...
async def write_chunks(queue: asyncio.Queue[dict], stop: asyncio.Event, stop_at: float | None) -> None:
    ...
    while not stop.is_set() or not queue.empty() or rows:
        ...
        if rows and (len(rows) >= MAX_ROWS or now - last_write >= FLUSH_SEC or (stop.is_set() and queue.empty())):
            write_parts(rows)
            write_log(f"wrote rows={len(rows)} queue={queue.qsize()}")
            rows = []
            last_write = now
        if expired(stop_at):
            stop.set()
...
def compact_hours(include_current: bool) -> None:
    ...
    for hour_dir in sorted(path for path in RAW_DIR.glob("*") if path.is_dir()):
        ...
        if not files:
            hour_dir.rmdir()
            continue
        out_path = MERGED_DIR / f"bidask1-{key}.parquet"
        ...
        table = table.sort_by([("ts_local_ns", "ascending"), ("venue", "ascending"), ("symbol", "ascending")])
        ...
        for path in files:
            path.unlink()
        hour_dir.rmdir()
        write_log(f"compacted hour={key} files={len(files)} rows={table.num_rows} out={out_path.name}")
...
if __name__ == "__main__":
    # 0 表示持续采集；临时测试可传秒数，例如：python collect_bidask1.py 120
