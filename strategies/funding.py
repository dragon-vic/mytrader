...
class Funding(Strategy):
    ...
    def on_stop(self) -> None:
        ...
        for kind in (self.PRE_TIMER, self.FREEZE_TIMER, self.RATE_TIMER, self.CLOSE_TIMER):
    ...
    def _schedule_next(self) -> None:
        ...
        self.clock.set_time_alert_ns(
            self._timer_name(self.CLOSE_TIMER),
            close_ns,
            callback=self._on_time,
            allow_past=False,
        )
    ...
    def _rate_list(self, limit: int, key: str = "rate") -> str:
        rows = [
            (ins_id, row)
            for ins_id, row in self.ins_map.items()
            if key in row
        ...
        with self.log_path.open("a", newline="", encoding="utf-8") as f:
            ...
            writer.writerow(
                [
                    ...
                    rate,
                    settle_rate,
                    self._side_name(side) if isinstance(side, OrderSide) else "",
                    pre,
                    self.notional,
                    fund_gain,
                    settle_gain,
